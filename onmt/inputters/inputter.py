...
from onmt.extended_torchtext.iterator import Iterator
...
def lazily_load_dataset(corpus_type, opt, task_id=0, is_log=True):
    """
    ...
    """
    assert corpus_type in ["train", "valid", "meta_valid"]

    def _lazy_dataset_loader(pt_file, corpus_type):
        dataset = torch.load(pt_file)
        if is_log:
            logger.info('(Task %d) Loading %s dataset from %s, number of examples: %d' %
                        (task_id, corpus_type, pt_file, len(dataset)))
        return dataset

    # Sort the glob output by file name (by increasing indexes).
    data_path = opt.data.split(',')[task_id]
...
def dataset_stats(corpus_type, opt, task_id=0):
    """
    Return statistic of the entire dataset
...
def load_fields(dataset, opt, checkpoint, task_id=0):
    ...
    if checkpoint is not None:
        logger.info('Loading vocab from checkpoint at %s.' % opt.warm_model)
        vocab = checkpoint['vocab']
    else:
        if opt.mtl_shared_vocab:
            data_path = opt.mtl_shared_vocab_path
        else:
            data_path = opt.data.split(',')[task_id]
