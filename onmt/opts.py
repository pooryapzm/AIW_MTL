...
def train_opts(parser):
    ...
    group.add('--warm_model', '-warm_model', default='', type=str,
              help="""If training from a checkpoint then this is the
                           path to the pretrained model's state_dict.""")
    ...
    group.add('--decay_method', '-decay_method', type=str, default="none",
              choices=['noam', 'performance', 'none'], help="Use a custom decay rate.")
...
def mtl_opts(parser):
    """ Multi-Task and Meta Learning options """
    group = parser.add_argument_group('MTL')
    group.add_argument('--mtl_schedule', '-mtl_schedule', type=int, default=0,
                       help="MTL training schedule")
    group.add_argument('--mtl_fully_share', '-mtl_fully_share', action="store_true",
                       help='share all of the parameters (WARNING: if True, it disregards all other sharing options)', default=False)
    group.add_argument('--mtl_shared_vocab', '-mtl_shared_vocab', action="store_true",
                       help='Is embeddings shared among all of the tasks?', default=False)
    group.add_argument('-mtl_shared_vocab_path', '--mtl_shared_vocab_path', default="",
                       help="Path to the MTL shared vocabulary file (.pt)")
    group.add_argument('--shared_enc_layers', '-shared_enc_layers', action='store', dest='shared_enc_layers', help='Shared encoder layers',
                        default="1")
    group.add_argument('--shared_dec_layers', '-shared_dec_layers', action='store', dest='shared_dec_layers', help='Shared decoder layers',
                        default="1")
    group.add_argument('--share_decoder_bottom', '-share_decoder_bottom', action="store_true",
                       help='Share bottom layers of the decoder (default is top)', default=False)
    group.add_argument('--share_encoder_bottom', '-share_encoder_bottom', action="store_true",
                       help='Share bottom layers of the encoder (default is top)', default=False)
    group.add_argument('--meta_batch_weighting', '-meta_batch_weighting', action="store_true",
                       help='Calculate Adaptive Importance Weights (AIWs)', default=False)
    group.add_argument('-normalize_meta_loss', '--normalize_meta_loss', action="store_true",
                       help='Normalize loss on the validation set (during calculation of AIWs)', default=False)
    group.add('--meta_mtl_concat_mode', '-meta_mtl_concat_mode', type=str, default="none",
              choices=['none', 'psuedo', 'concat'], help="Determines how mini-batches of different tasks "
                                                         "should be concated for computing AIWs at each update iteration")
    group.add_argument('--meta_batch_weighting_mode', '-meta_batch_weighting_mode', type=int, default=1,
                       help="determines how the final weights are caluclated w.r.t. AIWs:"
                            " 0: AIW 1: 1/normalization + AIW")
    group.add_argument('--meta_output', '-meta_output', default='',
                       help="""Path to output for the AIWs statistics (i.e. weight distribution info)""")
    group.add_argument('--meta_step_report', '-meta_step_report', type=int, default=2,
                       help="Report AIWs statistics every X steps")

    # TODO: Not implemented yet!
    group.add_argument('--mtl_shared_optimizer', '-mtl_shared_optimizer', action="store_true",
                       help='Use a single optimizer for all of the tasks',
                       default=False)
    group.add_argument('--mtl_gamma', '-mtl_gamma', action='store', dest='mtl_gamma', help='Gamma for MTL training',
