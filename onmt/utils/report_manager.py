...
class ReportMgrBase(object):
    ...
    def report_training(self, step, num_steps, learning_rate,
                        report_stats, multigpu=False, task_id=-1):
        """
        ...
        if step % self.report_every == 0:
            ...
            self._report_training(
                step, num_steps, learning_rate, report_stats, task_id)
    ...
    def report_step(self, lr, step, train_stats=None, valid_stats=None, task_id=0):
        """
        ...
        """
        self._report_step(
            lr, step, train_stats=train_stats, valid_stats=valid_stats, task_id=task_id)
...
class ReportMgr(ReportMgrBase):
    ...
    def _report_training(self, step, num_steps, learning_rate,
                         report_stats, task_id = -1):
        """
        See base class method `ReportMgrBase.report_training`.
        """
        report_stats.output(step, num_steps,
                            learning_rate, self.start_time, task_id)
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None, task_id=0):
        """
        See base class method `ReportMgrBase.report_step`.
        """
        if train_stats is not None:
            self.log('(Task %d) Train perplexity: %g' % (task_id, train_stats.ppl()))
            self.log('(Task %d) Train accuracy: %g' % (task_id, train_stats.accuracy()))
        ...
        if valid_stats is not None:
            self.log('(Task %d) Validation perplexity: %g' % (task_id, valid_stats.ppl()))
            self.log('(Task %d) Validation accuracy: %g' % (task_id, valid_stats.accuracy()))
