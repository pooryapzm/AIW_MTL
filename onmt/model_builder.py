...
from onmt.meta_modules import MetaModule, MetaLinear, MetaSequential
...
def build_decoder(opt, embeddings, main_decoder=None):
    """
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, main_model=None, task_id=0):
    """
    Args:
    ...
    if model_opt.model_type == "text":
        ...
        if model_opt.mtl_shared_vocab and main_model is not None:
            src_emb = main_model.encoder.embeddings
        else:
            src_emb = build_embeddings(model_opt, fields["src"], feat_fields)
        main_encoder = None
        if main_model:
            main_encoder = main_model.encoder
        encoder = build_encoder(model_opt, src_emb, main_encoder)
    ...
    if model_opt.mtl_shared_vocab and main_model is not None:
        tgt_emb = main_model.decoder.embeddings
    else:
        tgt_emb = build_embeddings(model_opt, fields["tgt"],
                                   feat_fields, for_encoder=False)
    ...
    main_decoder = None
    if main_model:
        main_decoder = main_model.decoder
    decoder = build_decoder(model_opt, tgt_emb, main_decoder=main_decoder)
    ...
    if not model_opt.copy_attn:
        ...
        #     gen_func
        # )
        generator = MetaSequential(
            MetaLinear(model_opt.dec_rnn_size, len(fields["tgt"].vocab), bias=True),
            gen_func
        )
...
def build_model(model_opt, opt, fields, checkpoint, main_model=None, task_id=0, is_meta_model=False):
    if not is_meta_model:
        logger.info('(Task %d) Building model...'%task_id)
    else:
        logger.info('(Task %d) Building meta model...' % task_id)
    model = build_base_model(model_opt, fields, use_gpu(opt), checkpoint, main_model=main_model, task_id=task_id)
    if not is_meta_model:
        logger.info(model)
    return model
